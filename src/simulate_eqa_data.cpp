...
List simulate_eqa_data(List parameters, int silence = 1){
  ...
  for(int i = 0; i < number_given_parameters; ++i){
    ...
    CharacterVector candidate_param(10);
    candidate_param[0] = "n";
    candidate_param[1] = "R";
    ...
    candidate_param[6] = "qpos";
    candidate_param[7] = "qran";
    candidate_param[8] = "prop";
    candidate_param[9] = "mmax";
  ...
  if(qran_exists == 1 and prop_exists == 1){
    Rcout << "Both qran and prop is found in parameters" << "\n";
  ...
  float sdx = average * cvx;
  float sdy = average * cvy;
    ...
    if(qran_exists == 1){
      if(qpos == 0){
        ...
        if(tau[i] <= limit){
          ...
          relocating_magnitude = R::rnorm(rel_diff * mmax * above, sdy / sqrt(R));
        }
      }
      else if(qpos == 1){
