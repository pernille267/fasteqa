...
//' }
 ...
 // [[Rcpp::export]]
 List estimate_absolute_dins(List data, int silence = 1) {
   ...
   for(int i = 0; i < n; ++i){
     // Extract necessary information on the ith sample (indices for replicated measurements)
   ...
   // Calculate varpar and then absolute_dins
   ...
   if((lambda < 1) & (!is_na_pooled_var_MS_A) & (!is_na_pooled_var_MS_B)){
     ...
     float absolute_dins = varpar - (var_MS_A * pow(b1, 2) + var_MS_B);
     
     List out = List::create(Named("absolute_dins") = absolute_dins);
     return out;
   }
     ...
     float absolute_dins = varpar - (var_MS_B * pow(b1, 2) + var_MS_A);
     List out = List::create(Named("absolute_dins") = absolute_dins);
     return out;
   }
     ...
     if(silence == -2){
       warning("Pooled variance of both MS_A and MS_B are NA values. absolute_dins could not be calculated!");  
     }
     List out = List::create(Named("absolute_dins") = NA_REAL);
     return out;  
   }
